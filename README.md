...
使用 `filter` 时不要同时传扁平字段。完整条件组查询产品 ID 为 `690dcb1b9c9dc8d0ff3c40eb`，固定返回第一页前50条；`pageIndex` 仅接受 `1`，`pageSize` 可保留默认值 `10` 或显式传 `50`。
...
通过高级筛选条件查询全国符合要求的企业清单。推荐直接传入 `filter` 条件组；完整条件组产品不执行真实分页，固定返回总命中数和第一页前50条企业。旧版扁平模式可分页获取最多500条。

**参数**:
- `filter` (推荐): JSON object 或 JSON 字符串
- `operStatus` / `industries` / `enterpriseType` / `name` 等旧版扁平参数同 `advanced_filter_get_enterprise_count`
...
- `pageIndex` / `pageSize`: 旧版扁平模式一页最大10条；`filter` 模式接受 `pageIndex=1`，以及默认 `pageSize=10` 或表示固定返回量的 `pageSize=50`，分页字段不会传给上游
...
MCP 保持 24 个 HandaaS 数据工具不变，另提供只读 Resources 帮助模型在组装 `filter` 前查询支持的维度。Resources 只公开随包发布的平台字段配置，不执行查询、不消耗产品调用次数，也不替代伴随 Skill 的条件规划。
...
2. **分页限制**: 列表类接口通常一页最多获取50条数据；旧版扁平高级筛选一页最大10条且最多获取500条。完整 `filter` 条件组固定返回总数和第一页前50条，可传 `pageIndex=1,pageSize=50`，但不执行真实分页
