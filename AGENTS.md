...
- Full high-screen condition mode sends exactly `filter`, `pageIndex`, and `pageSize` to product `690dcb1b9c9dc8d0ff3c40eb`. Forward validated pagination, require `pageIndex>=1`, and cap `pageSize` at 50. Legacy flat list mode remains capped at 10 rows per page and 500 rows total.
